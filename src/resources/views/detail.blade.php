...
<head>
    ...
    <title>Detail - {{ $alat->nama_alat }}</title>
    <!-- Tailwind CSS CDN -->
...
<body class="bg-slate-50 dark:bg-[#090d16] text-slate-900 dark:text-slate-100 antialiased min-h-screen pb-20 transition-colors duration-300">
    ...
    <header class="bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800/60 sticky top-0 z-40 shadow-sm transition-colors duration-300">
        <div class="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
            ...
            <a href="{{ url('/') }}" class="font-display font-black text-lg text-slate-900 dark:text-white flex items-center gap-2.5 tracking-tight">
            ...
            <!-- Back Button -->
            <a href="{{ url('/') }}" class="font-display inline-flex items-center gap-2 px-4 py-2.5 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold text-slate-600 dark:text-slate-400 bg-white dark:bg-[#131c31] hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-all duration-200 shadow-sm">
            ...
            <div class="md:col-span-7 space-y-6">
                <div>
                    ...
                    <p class="text-slate-500 dark:text-slate-400 text-sm leading-relaxed italic mt-3 border-l-4 border-blue-500 dark:border-blue-600 pl-4 bg-slate-50 dark:bg-slate-900/40 py-2 rounded-r-xl">
                        "{{ $alat->deskripsi ?? 'Tidak ada catatan deskripsi khusus untuk item ini.' }}"
                    </p>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div class="p-4 rounded-2xl border border-blue-100 dark:border-blue-900/30 bg-blue-50/30 dark:bg-blue-950/10 hover:shadow-md transition-all">
                        ...
                        <span class="font-display text-2xl font-black text-blue-700 dark:text-blue-400 block mt-1">
                            {{ $alat->jumlah }} <span class="text-xs font-bold text-slate-400 dark:text-slate-500">PCS</span>
                        </span>
                    </div>
                    <!-- Stat 2: Price -->
                    ...
                    <div class="p-4 rounded-2xl border border-slate-200/60 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/40 hover:shadow-md transition-all">
                        ...
                        <span class="font-display text-2xl font-black text-slate-900 dark:text-white block mt-1">
                            Rp{{ number_format(($alat->harga ?? 0) * ($alat->jumlah ?? 1), 0, ',', '.') }}
                        </span>
                    </div>
                </div>

                <!-- Bagian Grid Varian / Ukuran -->
                @if(isset($varian) && !$varian->isEmpty())
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                            @foreach($varian as $v)
                                <div class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 p-2.5 rounded-xl flex justify-between items-center hover:border-blue-400 dark:hover:border-blue-500/50 transition-all">
                                    <span class="font-display font-bold text-xs text-slate-700 dark:text-slate-300 truncate">{{ $v->nama_varian }}</span>
                                    <span class="bg-slate-900 dark:bg-slate-800 text-white dark:text-blue-400 font-display font-black text-[10px] px-2 py-0.5 rounded-md shadow-sm border border-transparent dark:border-slate-700">{{ $v->stok }} Pcs</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
                            ...
                            @foreach($riwayat as $log)
                                <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-900/30 transition-all duration-150">
                                    ...
                                    <td class="py-4 px-6 text-slate-400 dark:text-slate-500 font-medium">
                                        {{ $log->created_at ? \Carbon\Carbon::parse($log->created_at)->format('d M Y - H:i') : '-' }} WIB
                                    </td>
                                    
                                    <!-- Badge Classifier -->
                                    <td class="py-4 px-6 text-center">
                                        @if(strtolower($log->jenis_transaksi) === 'masuk')
                                    ...
                                    <!-- Asal / Tujuan -->
                                    <td class="py-4 px-6 font-bold text-blue-600 dark:text-blue-400 tracking-tight max-w-[180px] truncate" title="{{ $log->asal_atau_tujuan }}">
                                        {{ $log->asal_atau_tujuan ?? '-' }}
                                    </td>
                                    
                                    <!-- Keterangan -->
                                    <td class="py-4 px-6 text-slate-400 dark:text-slate-400 font-normal max-w-[240px] truncate" title="{{ $log->keterangan }}">
                                        {{ $log->keterangan ?? '-' }}
                                    </td>
                                </tr>
                            @endforeach
