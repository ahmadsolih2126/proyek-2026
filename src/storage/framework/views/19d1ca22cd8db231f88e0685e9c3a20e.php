    ...
    <div class="fixed top-6 right-6 z-50 space-y-3 max-w-sm w-full pointer-events-none">
        ...
        <div x-data="{ show: true, progress: 100 }" 
            ...
            <div class="flex-1">
                ...
                <p class="text-xs font-bold text-slate-700 dark:text-slate-300 mt-0.5 leading-relaxed"><?php echo e(session('success')); ?></p>
            </div>
        ...
        <?php if(session('transaksi_error')): ?>
        <div x-data="{ show: true, progress: 100 }" 
             x-show="show" 
             x-init="
                let interval = setInterval(() => { 
            ...
            <div class="flex-1">
                ...
                <p class="text-xs font-bold text-slate-700 dark:text-slate-300 mt-0.5 leading-relaxed"><?php echo e(session('transaksi_error')); ?></p>
            </div>
            <button @click="show = false" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 font-bold text-xs p-1">✕</button>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4 transition-all duration-300 hover:shadow-md">
                ...
                <div>
                    ...
                    <h4 class="text-xl font-black text-slate-900 dark:text-slate-100 mt-0.5"><?php echo e($semua_alat->count()); ?> <span class="text-xs text-slate-400 font-semibold uppercase">Jenis</span></h4>
                </div>
            </div>
            <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4 transition-all duration-300 hover:shadow-md">
                ...
                <div>
                    ...
                    <h4 class="text-xl font-black text-slate-900 dark:text-slate-100 mt-0.5"><?php echo e($semua_alat->sum('jumlah')); ?> <span class="text-xs text-slate-400 font-semibold uppercase">Unit Pcs</span></h4>
                </div>
            </div>
            <div class="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm flex items-center gap-4 transition-all duration-300 hover:shadow-md">
                ...
                <div>
                    ...
                    <h4 class="text-xl font-black text-indigo-600 dark:text-indigo-400 mt-0.5"><?php echo e($semua_pinjam->count()); ?> <span class="text-xs text-slate-400 font-semibold uppercase">Siswa</span></h4>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div class="lg:col-span-2 space-y-4">
                <div class="flex justify-between items-center mb-2">
                    ...
                    <span class="text-[10px] bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-full font-bold text-slate-500 dark:text-slate-400 uppercase"><?php echo e($semua_alat->count()); ?> Terdaftar</span>
                </div>

                <?php if($semua_alat->isEmpty()): ?>
                ...
                <?php else: ?>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <?php $__currentLoopData = $semua_alat; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $alat): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            
                            <div x-show="'<?php echo e(Str::lower($alat->nama_alat)); ?>'.includes(searchKeyword.toLowerCase())"
                                ...
                                <div class="px-5 py-4 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50 border-b border-slate-100 dark:border-slate-800">
                                    ...
                                    <div class="flex items-center gap-1">
                                        <?php if($is_logged_in): ?>
                                            <button onclick="openEditModal(this)" data-id="<?php echo e($alat->id); ?>" data-nama="<?php echo e($alat->nama_alat); ?>" data-kategori="<?php echo e($alat->kategori); ?>" data-harga="<?php echo e($alat->harga); ?>" class="p-1.5 text-xs text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-white dark:hover:bg-slate-800 rounded-lg shadow-sm transition-all hover:scale-110" title="Edit">Ubah</button>
                                            <form action="/alat-hapus/<?php echo e($alat->id); ?>" method="POST" onsubmit="return confirm('Hapus data ini bray?')" class="inline">
                                                <?php echo csrf_field(); ?>
                                ...
                                <div class="p-5 flex-1 flex flex-col justify-between">
                                    <div>
                                        ...
                                        <div class="mt-4 grid grid-cols-2 gap-3">
                                            <div class="p-3 rounded-xl border flex flex-col justify-between transition-all duration-300
                                                ...
                                                <p class="text-[9px] font-bold uppercase tracking-wider
                                                    <?php if($alat->jumlah == 0): ?> text-rose-500 dark:text-rose-400
                                                    <?php elseif($alat->jumlah <= 5): ?> text-amber-600 dark:text-amber-400
                                                    <?php else: ?> text-emerald-600 dark:text-emerald-400 <?php endif; ?>">
                                                    Status Stok
                                                </p>
                                                <p class="text-base font-black mt-0.5 
                                                    <?php if($alat->jumlah == 0): ?> text-rose-700 dark:text-rose-300
                                                    <?php elseif($alat->jumlah <= 5): ?> text-amber-700 dark:text-amber-300
                                                    <?php else: ?> text-emerald-700 dark:text-emerald-300 <?php endif; ?>">
                                                    <?php echo e($alat->jumlah); ?> 
                                                    <span class="text-[10px] font-bold uppercase">
                                                        <?php if($alat->jumlah == 0): ?> HABIS <?php elseif($alat->jumlah <= 5): ?> KRITIS <?php else: ?> Pcs <?php endif; ?>
                                                    </span>
                                                </p>
                                            </div>
                                            ...
                                            <div class="bg-slate-50/70 dark:bg-slate-900/40 p-3 rounded-xl border border-slate-100/50 dark:border-slate-800/60">
                                                ...
                                                <p class="text-sm font-extrabold text-slate-700 dark:text-slate-300 mt-1">Rp <?php echo e(number_format($alat->harga, 0, ',', '.')); ?></p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="mt-4 pt-1 flex items-center gap-1.5 flex-wrap">
                                            ...
                                            <?php if($alat->kategori): ?>
                                                <?php if(Str::contains(Str::lower($alat->kategori), ['bola', 'futsal', 'sepak'])): ?>
                                                    <span class="px-2.5 py-0.5 text-[10px] font-bold rounded-full bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-900/30 uppercase tracking-wide"><?php echo e($alat->kategori); ?></span>
                                                <?php elseif(Str::contains(Str::lower($alat->kategori), ['atletik', 'lari', 'lompat'])): ?>
                                                    <span class="px-2.5 py-0.5 text-[10px] font-bold rounded-full bg-amber-50 dark:bg-amber-950/50 text-amber-600 dark:text-amber-400 border border-amber-100 dark:border-amber-900/30 uppercase tracking-wide"><?php echo e($alat->kategori); ?></span>
                                                <?php elseif(Str::contains(Str::lower($alat->kategori), ['senam', 'fitness', 'gym'])): ?>
                                                    <span class="px-2.5 py-0.5 text-[10px] font-bold rounded-full bg-purple-50 dark:bg-purple-950/50 text-purple-600 dark:text-purple-400 border border-purple-100 dark:border-purple-900/30 uppercase tracking-wide"><?php echo e($alat->kategori); ?></span>
                                                <?php else: ?>
                                                    <span class="px-2.5 py-0.5 text-[10px] font-bold rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700/60 uppercase tracking-wide"><?php echo e($alat->kategori); ?></span>
                                                <?php endif; ?>
                                            <?php else: ?>
            ...
            <div class="space-y-6">
                <div>
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-[10px] bg-indigo-50 dark:bg-indigo-950/40 px-2.5 py-1 rounded-full font-bold text-indigo-600 dark:text-indigo-400 uppercase"><?php echo e($semua_pinjam->count()); ?> Siswa</span>
                    </div>
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 p-4 shadow-sm space-y-3 max-h-[380px] overflow-y-auto custom-scrollbar">
                        <?php if($semua_pinjam->isEmpty()): ?>
                            <p class="text-xs font-bold text-slate-400 p-4 text-center uppercase">Belum Ada Peminjaman.</p>
                        <?php else: ?>
                            <?php $__currentLoopData = $semua_pinjam; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $pinjam): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="p-3 bg-slate-50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-800 rounded-2xl flex flex-col justify-between gap-2 transition-all hover:shadow-sm">
                                    <div>
                                        <div class="flex justify-between items-start">
                                            <h4 class="text-xs font-black text-slate-900 dark:text-slate-200 uppercase tracking-tight"><?php echo e($pinjam->nama_alat); ?></h4>
                                            <span class="px-2 py-0.5 bg-indigo-100 dark:bg-indigo-950 text-indigo-800 dark:text-indigo-300 rounded font-black text-[9px]"><?php echo e($pinjam->jumlah); ?> PCS</span>
                                        </div>
                                        <p class="text-[11px] text-slate-500 dark:text-slate-400 font-bold mt-1">Peminjam: <span class="text-blue-600 dark:text-blue-400 uppercase font-black"><?php echo e($pinjam->nama_peminjam); ?></span></p>
                                        <p class="text-[9px] text-slate-400 dark:text-slate-500 font-semibold mt-0.5"><?php echo e(\Carbon\Carbon::parse($pinjam->created_at)->diffForHumans()); ?></p>
                                    </div>
                                    <?php if($is_logged_in): ?>
                                        <form action="/peminjaman-kembali/<?php echo e($pinjam->id); ?>" method="POST" class="w-full">
                                            <?php echo csrf_field(); ?>
                ...
                <div>
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 p-4 shadow-sm space-y-3 max-h-[400px] overflow-y-auto custom-scrollbar">
                        <?php if($semua_log->isEmpty()): ?>
                            <p class="text-xs font-bold text-slate-400 p-4 text-center uppercase">Belum ada aktivitas log.</p>
                        <?php else: ?>
                            <?php $__currentLoopData = $semua_log; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $log): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="p-3 border-b border-slate-50 dark:border-slate-800 last:border-0 flex gap-3 items-start transition-all hover:bg-slate-50/50 dark:hover:bg-slate-800/30 rounded-xl">
                                    <div class="text-xs font-bold text-slate-400">
                                        <?php if($log->jenis_transaksi === 'masuk'): ?> In <?php else: ?> Out <?php endif; ?>
                                    </div>
                                    <div class="flex-1 min-w-0">
                                        <div class="flex justify-between items-start">
                                            <p class="text-xs font-black text-slate-900 dark:text-slate-200 truncate uppercase tracking-tight"><?php echo e($log->nama_alat ?? 'Alat Terhapus'); ?></p>
                                            <span class="text-[10px] font-black <?php echo e($log->jenis_transaksi === 'masuk' ? 'text-emerald-600' : 'text-rose-600'); ?> uppercase ml-2">
                                                <?php echo e($log->jenis_transaksi === 'masuk' ? '+' : '-'); ?><?php echo e($log->jumlah); ?>

                                            </span>
                                        </div>
                                        <p class="text-[10px] text-slate-400 dark:text-slate-500 font-medium mt-0.5 uppercase tracking-wide">📍 <?php echo e($log->asal_atau_tujuan ?? '-'); ?></p>
                                        <p class="text-[10px] text-slate-400 dark:text-slate-400 font-semibold bg-slate-50 dark:bg-slate-950/40 p-1.5 rounded-lg border border-slate-100 dark:border-slate-800 mt-1 italic">"<?php echo e($log->keterangan ?? '-'); ?>"</p>
                                        
                                        <p class="text-[8px] text-slate-400 dark:text-slate-500 font-semibold mt-1 flex items-center gap-1">
                                            <span>Waktu:</span> <?php echo e(\Carbon\Carbon::parse($log->created_at)->diffForHumans()); ?>
            ...
            <form action="/transaksi-proses" method="POST" class="space-y-4">
                ...
                <div id="form_alat_lama">
                    ...
                    <select name="alat_olahraga_id" id="select_alat_olahraga_id" class="w-full p-3 bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-xl font-bold text-xs text-slate-700 dark:text-slate-300 uppercase focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <?php $__currentLoopData = $semua_alat; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <option value="<?php echo e($a->id); ?>"><?php echo e($a->nama_alat); ?></option>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </select>
                </div>
